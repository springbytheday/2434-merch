...
async function onSignedIn(user) {
  ...
  currentUser.username = user.user_metadata?.username
    || user.email.replace('@merchrchive.local', '');
...
async function saveItem() {
  ...
  const payload = {
    liver:    name,
    series:   document.getElementById('fGroup').value.trim(),
...
function getFiltered() {
  ...
  return allItems.filter(item => {
    ...
    const matchGroup  = !groupFilter || item.series === groupFilter;
...
function updateStats() {
  ...
  document.getElementById('statSpentWrap').style.display    = loggedIn ? '' : 'none';
  ...
  if (loggedIn) {
    ...
    document.getElementById('statSpent').textContent =
      Object.entries(byCurrency).map(([c,v]) => `${fmtNum(v)} ${c}`).join(' / ') || '—';
  }
}
...
function updateGroupFilter() {
  ...
  const prev = gf.value;
  const groups = [...new Set(allItems.map(i => i.series).filter(Boolean))].sort();
...
function openEdit(id) {
  ...
  document.getElementById('fName').value     = item.liver    || '';
  document.getElementById('fGroup').value    = item.series   || '';
